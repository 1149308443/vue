      ...
      >
        ...
        >
          ...
          <div
            id="uprightbtn"
            class="uprightbtn"
            @mousedown="onUpRightbtn"
          ></div>
          <div
            id="downleftbtn"
            class="downleftbtn"
            @mousedown="onDownleftbtn"
          ></div>
          <div
            id="downrightbtn"
            class="downrightbtn"
            @mousedown="onDownRightbtn"
          ></div>
        </div>
      </div>
  ...
  methods: {
    ...
    mouseMove(e) {
      ...
      if (this.boxId === "movingBox" && this.dragging) {
        let realTop =
          e.offsetY + e.target.offsetTop - this.diffY > 0
            ? e.offsetY + e.target.offsetTop - this.diffY
            : 0;
        let realLeft =
          e.offsetX + e.target.offsetLeft - this.diffX > 0
            ? e.offsetX + e.target.offsetLeft - this.diffX
            : 0;
    ...
    mouseUp() {
      ...
      if (this.boxId === "activeBox") {
        if (this.$refs.box) {
          this.boxId = "changeBox";
          if (
            this.$refs.box.offsetWidth < 3 ||
            this.$refs.box.offsetHeight < 3
          ) {
            this.boxVisible = false;
            this.boxId = "";
          }
    ...
    getHotData() {
      ...
      if (target) {
        ...
        let data = [
          ...
          [
            this.toFixed6(offsetLeft + width, WIDTH),
            this.toFixed6(offsetTop, HEIGHT),
          ],
          [
            this.toFixed6(offsetLeft + width, WIDTH),
            this.toFixed6(offsetTop + height, HEIGHT),
          ],
          [
            this.toFixed6(offsetLeft, WIDTH),
            this.toFixed6(offsetTop + height, HEIGHT),
          ],
        ];
        // 矩形中点
...
.rectangle-marker {
  ...
  .fade-enter-active {
    animation: hide-and-show 0.5s;
  }
  .fade-leave-active {
    animation: hide-and-show 0.5s reverse;
  }
